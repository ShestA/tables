...
drawGameApp dgaData = Pictures [result, infopic, opportunitypic, pospic, deletepic] where
...
handleGameEvent (EventKey (MouseButton LeftButton) Down _ (hgeMX, hgeMY)) x = xnew where
    ...
    hgeNewBoard = attachMovingChecker (hgeMX / hgeSX, hgeMY / hgeSY) (adBoard x)
...
handleGameEvent (EventKey (MouseButton LeftButton) Up _ (hgeMX, hgeMY)) x = xnew where
    ...
    xnew        = ApplicationData (adScale x) (adGen x) hgeNewBoard
handleGameEvent (EventKey (SpecialKey KeyDelete) Down _ _) x = xnew where
    hgeNewBoard = tryDeleteChecker (adBoard x)
    xnew        = ApplicationData (adScale x) (adGen x) hgeNewBoard
...
handleGameEvent (EventKey (SpecialKey KeyEnter) Down _ _) x = xnew where
    xnew        = ApplicationData (adScale x) (adGen x) (tryStepped (adBoard x))
